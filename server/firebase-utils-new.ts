...
export const deleteFile = async (fileUrl: string): Promise<boolean> => {
  ...
  try {
    ...
    if (!fileUrl.includes('firebasestorage.googleapis.com') && 
      ...
      // محاولة التعامل مع المسار المباشر كعنوان محلي محتمل
      try {
...
export const getFileType = (fileType: string): string => {
  // استخراج النوع الأساسي من سلسلة نوع الملف (مثال: 'image/jpeg' سترجع 'image')
...
export const getReadableFileSize = (sizeInBytes: number): string => {
  ...
  if (sizeInBytes < KB) {
    ...
    return `${Math.round(sizeInBytes / KB * 10) / 10} كيلوبايت`;
  } else if (sizeInBytes < GB) {
    return `${Math.round(sizeInBytes / MB * 10) / 10} ميجابايت`;
  } else {
    return `${Math.round(sizeInBytes / GB * 10) / 10} جيجابايت`;
  }
};