...
try {
  ...
  console.warn('⚠️  cors package not found, using manual CORS setup');
}
...
app.get('/api/test-supabase', handleAsync(async (req, res) => {
  ...
  try {
    ...
    try {
      ...
      tests.auth = true; // إذا لم يكن هناك خطأ
    } catch (err) {
      tests.auth = false;
    }
...
app.post('/api/upload', handleAsync(async (req, res) => {
  // هذا مثال بسيط - في الواقع تحتاج إلى multer أو مكتبة مشابهة
  res.json({ message: 'File upload endpoint - requires multer implementation' });
}));

// ===== CATCH-ALL ROUTE =====
// لتطبيقات SPA - إعادة توجيه جميع الطلبات إلى index.html
app.get('*', (req, res) => {
...
const server = app.listen(PORT, () => {
  ...
  console.log(`🔒 الحماية: ${cors ? '✅ CORS' : '⚠️  Manual CORS'} | Rate Limiting | Security Headers`);
