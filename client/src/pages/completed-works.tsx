...
import { z } from "zod";
import { Plus, Upload, Edit, Trash2, FileText, Calendar, DollarSign, Tag } from "lucide-react";
  ...
  return (
    <div className="container mx-auto p-6 space-y-6" dir="rtl">
      <div className="flex justify-between items-center">
        <div>
          <h1 className="text-3xl font-bold">الأعمال المنجزة</h1>
          <p className="text-muted-foreground">قسم مستقل للمدير لإدارة الأعمال والمستندات</p>
        </div>
      ...
      <Tabs defaultValue="works" className="w-full">
                  ...
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      ...
                      <div className="flex gap-1">
                        <Button size="sm" variant="ghost" onClick={() => deleteWorkMutation.mutate(work.id)}>
        ...
        <TabsContent value="documents" className="space-y-4">
          {documentsLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
              {documents.map((doc: CompletedWorksDocument) => (
                <Card key={doc.id} className="hover:shadow-md transition-shadow">
                  <CardHeader>
                    <div className="flex justify-between items-start">
                      <CardTitle className="text-lg">{doc.title}</CardTitle>
                      <Button size="sm" variant="ghost" onClick={() => deleteDocumentMutation.mutate(doc.id)}>
                        <Trash2 className="h-4 w-4" />
                      </Button>
                    </div>
                    <div className="flex gap-2">
